...
for feature_group in feature_groups_single:
    ...
    X = pd.read_csv(f"../../Features/Topt/{feature_group}/train.csv", header=None).values
    scaler = MinMaxScaler()
    X = scaler.fit_transform(X)
    feature_arrays.append(X)

X = pd.read_csv(f"../../Features/Topt/esmc_6b/embeddings_train.csv", header=None).values
...
print(X.shape)
y = np.load(f"../../Features/topt_train_labels.npy")

X_base, X_meta, y_base, y_meta = train_test_split(X, y, test_size=0.4, random_state=42)

base_model_1 = SVR(kernel='rbf', C=10, epsilon=0.1, gamma='scale')
...
meta_model = RandomForestRegressor(random_state=42, n_jobs=-1, max_depth=10, n_estimators=100)
