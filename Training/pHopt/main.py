...
feature_groups_single = ['PAAC', 'DPC', 'SOCNumber']
...
for feature_group in feature_groups_single:
    ...
    X = pd.read_csv(f"../../Features/pHopt/{feature_group}/train.csv", header=None).values
    scaler = MinMaxScaler()
    X = scaler.fit_transform(X)
    feature_arrays.append(X)

X = pd.read_csv(f"../../Features/pHopt/ankh/embeddings_train.csv", header=None).values
...
print(X.shape)
y = np.load(f"../../Features/pHopt_train_labels.npy")

X_base, X_meta, y_base, y_meta = train_test_split(X, y, test_size=0.4, random_state=42)

base_model_1 = RandomForestRegressor(random_state=42, bootstrap=True, min_samples_split=2, n_estimators=100, n_jobs=-1, max_depth=None)
